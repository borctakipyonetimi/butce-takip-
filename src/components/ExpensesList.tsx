...
import { PlusCircle, ShoppingCart, Folder, Edit, Trash2, Calendar, ClipboardList, BarChart3, Check, AlertTriangle, Sparkles, X, Lightbulb } from "lucide-react";
...
const getSavingTipForCategory = (name: string, icon: string): string => {
  ...
  // Keyword mapping
  if (norm.includes("market") || norm.includes("mutfak") || norm.includes("gıda") || norm.includes("bakkal") || norm.includes("manav") || icon === "🛒") {
  ...
  if (norm.includes("yemek") || norm.includes("restoran") || norm.includes("kafe") || norm.includes("burger") || norm.includes("kebap") || norm.includes("dışarı") || icon === "🍔" || icon === "🥩" || icon === "🍷") {
  ...
  if (norm.includes("kira") || norm.includes("ev") || norm.includes("konut") || norm.includes("site") || norm.includes("aidat") || icon === "🏠") {
  ...
  if (norm.includes("ulaşım") || norm.includes("yol") || norm.includes("otobüs") || norm.includes("metro") || norm.includes("taksi") || icon === "🚗" || icon === "✈️") {
  ...
  if (norm.includes("araba") || norm.includes("araç") || norm.includes("yakıt") || norm.includes("akaryakıt") || norm.includes("benzin") || norm.includes("bakım") || norm.includes("sigorta") || icon === "🔧") {
  ...
  if (norm.includes("fatura") || norm.includes("elektrik") || norm.includes("su") || norm.includes("doğalgaz") || norm.includes("internet") || norm.includes("telefon") || icon === "⚡") {
  ...
  if (norm.includes("giyim") || norm.includes("elbise") || norm.includes("ayakkabı") || norm.includes("moda") || norm.includes("alışveriş") || icon === "🎒" || icon === "🛍️" || icon === "💇") {
  ...
  if (norm.includes("eğlence") || norm.includes("sinema") || norm.includes("konser") || norm.includes("tiyatro") || norm.includes("aktivite") || norm.includes("oyun") || icon === "🍿" || icon === "🎸") {
  ...
  if (norm.includes("sağlık") || norm.includes("ilaç") || norm.includes("hastane") || norm.includes("doktor") || norm.includes("eczane") || icon === "💊") {
  ...
  if (norm.includes("eğitim") || norm.includes("kurs") || norm.includes("kitap") || norm.includes("okul") || icon === "🎓") {
  ...
  if (norm.includes("hediye") || norm.includes("bağış") || norm.includes("yardım") || icon === "🎁" || icon === "💰") {
...
export const ExpensesList: React.FC<ExpensesListProps> = ({
  ...
  const [showTipCategory, setShowTipCategory] = useState<ExpenseCategory | null>(null);
  ...
  const handleOpenEditExpense = (e: Expense) => {
    ...
    setDate(e.date ? e.date.slice(0, 10) : new Date().toISOString().slice(0, 10));
    setIsExpModalOpen(true);
  };
  ...
  const handleRandomizeColors = () => {
    try {
      if (!onUpdateAllCategories) {
        console.warn("onUpdateAllCategories prop is missing in ExpensesList! Fallback to local array alert.");
        return;
      }
      const palette = [
        ...
        "#84cc16", "#0284c7", "#4f46e5", "#b91c1c", "#0d9488"
      ];
  ...
  const handleDrop = (e: React.DragEvent, targetIndex: number) => {
    e.preventDefault();
    if (draggedIndex === null || draggedIndex === targetIndex || !onUpdateAllCategories) return;
  ...
  const doughnutData = expenseCategories.map((c, idx) => ({
    label: c.name,
  ...
  expenseCategories.forEach((cat) => {
    categoryCurrentMonthTotals[cat.id] = expenses
      .filter((e) => {
        ...
        const eDate = new Date(e.date);
        return eDate.getFullYear() === currentYear && eDate.getMonth() === currentMonthIndex && e.categoryId === cat.id;
      })
      .reduce((sum, item) => sum + item.amount, 0);
  });
  ...
  const selectedFilterCategory = expenseCategories.find((c) => c.id === selectedFilterCategoryId);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 justify-between sm:flex-row sm:items-center">
        <h2 className="text-lg font-bold flex items-center gap-2 text-slate-800 dark:text-slate-100">
          <ShoppingCart className="w-5 h-5 text-rose-500" /> AYLIK HARCAMA GİDERLERİ
        </h2>
      ...
      <div className="p-4 bg-rose-50/50 dark:bg-rose-950/20 text-rose-950 dark:text-rose-300 rounded-2xl flex items-center justify-between font-bold text-xs">
        ...
        <span className="text-base text-rose-600 dark:text-rose-400 font-mono">{format(totalExpenses)}</span>
      </div>
        ...
        <div className="space-y-4 shadow-sm rounded-2xl">
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <h4 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1">
                <ClipboardList className="w-4 h-4 text-rose-500" /> HARCAMA KAYITLARI
              </h4>
              <select
                value={selectedFilterCategoryId || ""}
                onChange={(e) => setSelectedFilterCategoryId(e.target.value ? parseInt(e.target.value) : null)}
            ...
            >
              ...
              <div
                ...
                style={{
                  backgroundColor: selectedFilterCategory ? selectedFilterCategory.color : "#6366f1",
                }}
              />
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <h5 className="text-xs sm:text-sm font-black text-slate-800 dark:text-slate-100 transition-all duration-300">
                      {selectedFilterCategory ? `${selectedFilterCategory.icon || "🛒"} ${selectedFilterCategory.name}` : "Tüm Kategoriler"}
                    </h5>
                  </div>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold flex items-center gap-1.5 pt-0.5">
                    ...
                    <span>•</span>
                    <span className="font-bold text-rose-500 dark:text-rose-400">Toplamın %{percentageOfTotal}'i</span>
                  </p>
                </div>

                <div className="text-right space-y-0.5 shrink-0">
                  <span className="text-[9px] font-black tracking-widest text-slate-400 dark:text-slate-500 block uppercase">Tutar</span>
                  <motion.div
                    ...
                    style={{
                      color: selectedFilterCategory ? selectedFilterCategory.color : "#e11d48",
                    }}
                  >
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-700/50 h-1.5 rounded-full mt-3 overflow-hidden">
                <motion.div
                  ...
                  style={{
                    backgroundColor: selectedFilterCategory ? selectedFilterCategory.color : "#e11d48",
                  }}
                />
              </div>
            </motion.div>
                ...
                {filteredExpenses.map((e) => {
                  ...
                  return (
                    <motion.div
                      ...
                      transition={isNew ? { duration: 0.5, ease: "easeOut" } : { type: "spring", stiffness: 350, damping: 25 }}
                    ...
                    >
                      ...
                      {isNew && (
                        <motion.div
                          initial={{ left: "-100%" }}
                          animate={{ left: "100%" }}
                          transition={{ repeat: Infinity, repeatType: "loop", duration: 1.6, ease: "linear" }}
                      ...
                      <div className="space-y-1 relative z-10">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {cat ? `${cat.icon || "🛒"} ${cat.name}` : "Kategorisiz"}
                          </span>
                        ...
                        <p className="text-[10px] text-slate-400 flex items-center gap-0.5 font-semibold">
                          <Calendar className="w-3 h-3" /> {new Date(e.date).toLocaleDateString("tr-TR")}
                        </p>
                      </div>
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wide">KATEGORİLERİ DÜZENLE</h4>
            ...
            <div id="expenses-category-list-container" className="flex flex-col gap-3 animate-fade-in bg-slate-100/40 dark:bg-slate-900/40 p-3 sm:p-4 rounded-3xl border border-slate-200/50 dark:border-slate-800/60 shadow-inner w-full">
              <div className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 select-none">
                <div className="flex items-center gap-3">
                  <span>
                    Tanımlı Kategori Sayısı: <span className="text-indigo-600 dark:text-indigo-400 font-mono font-bold">{expenseCategories.length}</span>
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                      const config = expenseCategories.map(({ name, color }) => ({ name, color }));
                ...
                <button
                  type="button"
                  onClick={() => setIsInlineEditingCategory(!isInlineEditingCategory)}
              ...
              {budgetGoal > 0 && currentMonthExpensesTotal >= budgetGoal * 0.9 && (
                <motion.div
                ...
                >
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-[11px] text-slate-700 dark:text-slate-300 font-semibold leading-relaxed">
                      Mevcut ayın bütçe hedefinin (<span className="font-mono font-black">{format(budgetGoal)}</span>) <span className="text-rose-600 dark:text-rose-400 font-extrabold">%90'ını</span> geçtiniz! 
                      Toplam aylık harcama: <span className="font-mono font-black text-rose-600 dark:text-rose-400">{format(currentMonthExpensesTotal)}</span> (Hedefe Oranı: <span className="font-mono font-black">%{((currentMonthExpensesTotal / budgetGoal) * 100).toFixed(1)}</span>). Bütçenizi kontrol altında tutmanızı öneririz.
                    </p>
                  </div>
                </motion.div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 w-full">
                ...
                return (
                  <motion.div
                     ...
                     draggable={!isInlineEditingCategory && !!onUpdateAllCategories}
                     ...
                     whileHover={isInlineEditingCategory ? {} : {
                       scale: 1.04,
                       backgroundColor: isSelected
                         ? `${c.color || "#6366f1"}35`
                         : `${c.color || "#6366f1"}15`,
                       filter: "brightness(0.93) contrast(1.05) saturate(1.12)",
                       boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.08), 0 4px 6px -2px rgba(0, 0, 0, 0.04)"
                     }}
                     whileTap={isInlineEditingCategory ? {} : { scale: 0.98 }}
                     className={`relative group flex items-center justify-between gap-2 px-3 py-2 text-slate-700 dark:text-slate-200 border-l-[4px] rounded-xl text-xs font-semibold select-none ${
                       isSelected
                       ...
                       isInlineEditingCategory ? "shadow-md ring-1 ring-indigo-500/10" : "cursor-pointer"
                     } ${
                       isDragged ? "opacity-30 border-dashed border-indigo-400 bg-indigo-50/10 scale-95" : ""
                     } ${
                       isOver && !isDragged ? "border-indigo-500 ring-2 ring-indigo-500/20 scale-105" : ""
                     }`}
                     style={{
                       borderLeftColor: c.color || "#6366f1",
                       backgroundColor: isSelected
                         ? `${c.color || "#6366f1"}25`
                         : (isDragged ? undefined : `${c.color || "#6366f1"}03`),
                       borderTopColor: isSelected ? c.color : undefined,
                       borderRightColor: isSelected ? c.color : undefined,
                       borderBottomColor: isSelected ? c.color : undefined
                     }}
                     title={
                       isInlineEditingCategory 
                  ...
                  >
                     ...
                     {!isInlineEditingCategory && (
                       <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-950/95 dark:bg-slate-900/95 text-white text-[10.5px] rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none whitespace-nowrap z-[100] shadow-2xl border border-indigo-500/15 flex flex-col items-center gap-0.5 animate-fade-in-fast">
                         <span className="text-slate-400 text-[8px] tracking-widest font-black uppercase">Bu Ayın Toplamı</span>
                         <span className="text-emerald-400 font-black text-xs font-mono">{format(currentMonthTotal)}</span>
                     ...
                     {isInlineEditingCategory ? (
                       <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                         <span className="text-sm select-none shrink-0" title="Kategori simgesi">{c.icon || "🛒"}</span>
                       ...
                       <>
                         <div className="flex items-center gap-2 flex-1 min-w-0">
                           ...
                           />
                           <span className="text-sm select-none shrink-0" title={`${c.name} simgesi`}>{c.icon || "🛒"}</span>
                           <span className={`truncate text-xs font-bold leading-none ${isSelected ? "text-indigo-950 dark:text-indigo-250" : "text-slate-705 dark:text-slate-250"}`} title={c.name}>
                             {c.name}
                           </span>
                         </div>
                         ...
                         <button
                           ...
                           onClick={(e) => {
                             e.stopPropagation();
                             setShowTipCategory(showTipCategory?.id === c.id ? null : c);
                           }}
              ...
              {(() => {
                const selectedCat = expenseCategories.find(c => c.id === selectedFilterCategoryId);
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <h5 className="text-[11px] font-black text-amber-800 dark:text-amber-400 uppercase tracking-wider">
                          {selectedCat.icon || "🔑"} {selectedCat.name} Tasarruf İpucu
                        </h5>
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-350 font-semibold leading-relaxed">
                        {getSavingTipForCategory(selectedCat.name, selectedCat.icon || "🛒")}
                      </p>
                    </div>
                  </motion.div>
                );
        ...
        <div className="space-y-6">
          ...
          {doughnutData.length > 0 ? (
            <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-2">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wide text-center">Gider Dağılım Grafiği</h4>
          ...
          <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-4">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
              <BarChart3 className="w-4 h-4 text-rose-500" /> Aylık Harcama Değişim Analizi
            </h4>
            <div className="pt-2">
              <BarChart data={last6MonthsData} />
            </div>
            <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">
              * Son 6 aya ait harcamalarınızın aylık toplam değişim trendini gösterir.
            </p>
          </div>
        </div>
      </div>
      ...
      {isExpModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-3xl p-6 w-full max-w-sm space-y-4 shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-400 block mb-1">KATEGORİ SEÇİN</label>
                <select
                  value={categoryId}
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 block mb-1">HARCAMA TUTARI</label>
                <input
                  type="number"
                  value={amount}
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 block mb-1">HARCAMA AÇIKLAMASI</label>
                <input
                  type="text"
                  value={description}
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 block mb-1">HARCAMA TARİHİ</label>
                <input
                  type="date"
                  value={date}
      ...
      {isCatModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-3xl p-6 w-full max-w-sm space-y-4 shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-400 block mb-1">KATEGORİ ADI</label>
                <input
                  type="text"
                  value={categoryName}
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 block mb-1">KATEGORİ RENGİ</label>
                <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[8px] sm:text-[9px] font-black text-slate-400 font-mono tracking-wider uppercase">Seçilen Renk Kodu</span>
                    <input
                      type="text"
                      maxLength={7}
                      value={categoryColor}
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 block mb-1">KATEGORİ SİMGESİ</label>
            ...
            <div className="flex items-center justify-between border-b pb-3 dark:border-slate-700">
              <div className="flex items-center gap-2">
                <div className="p-2.5 bg-amber-50 dark:bg-amber-950/60 text-amber-500 rounded-xl">
                  <span className="text-xl select-none leading-none inline-block">{showTipCategory.icon || "💡"}</span>
                </div>
                <div>
            ...
            <div className="space-y-3">
              <div className="p-4 bg-amber-50/55 dark:bg-amber-950/20 border-l-[3px] border-amber-500 rounded-2xl space-y-2">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 font-semibold leading-relaxed">
                  {getSavingTipForCategory(showTipCategory.name, showTipCategory.icon || "🛒")}
                </p>
              </div>

              <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold leading-normal italic text-center">
                🤖 Analiz motorumuz bu tavsiyeyi kategori profiline özel üretmiştir.
              </p>
            </div>
